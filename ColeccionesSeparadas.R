...
########## MODIFICAR NOMBRES DE LAS COLECCIONES (opcional) ##########
nombreColeccionInformacionTaxonomica <- "colSepInformacionTaxonomica"
nombreColeccionRegistroDeOcurrencia <- "colSepRegistroDeOcurrencia"
nombreColeccionPoblacion <- "colSepPoblacion"
########## MODIFICAR NOMBRES DE LAS COLECCIONES (opcional) ##########
...
if (!requireNamespace("httr", quietly = TRUE)) install.packages("httr", dependencies = TRUE)
library(httr)

if (!requireNamespace("jsonlite", quietly = TRUE)) install.packages("jsonlite", dependencies = TRUE)
library(jsonlite)
...
insertarDatos <- function(df, nombreColeccion = NULL) {
  if (!is.null(nombreColeccion)) {
    df$nombreColeccion <- nombreColeccion
  }
  
  rowsPerPart <- ceiling(nrow(df) / numParts)
  splitData <- split(df, rep(1:numParts, each = rowsPerPart, length.out = nrow(df)))
...
####################################
# COLECCIÓN INFORMACIÓN TAXONÓMICA #
####################################
...
insertarDatos(InformacionTaxonomica, nombreColeccionInformacionTaxonomica)

rm(InformacionTaxonomica)

####################################
# COLECCIÓN REGISTRO DE OCURRENCIA #
####################################
...
insertarDatos(RegistroDeOcurrencia, nombreColeccionRegistroDeOcurrencia)

rm(RegistroDeOcurrencia)
...
rm(dfPoblacion, InventarioNacional, Gestion, MedidasDeConservación, ConservacionExSitu, Links, convertirEnLista, indices)

##########################
### INSERTAR COLECCIÓN ###
##########################

insertarDatos(Poblacion, nombreColeccionPoblacion)

rm(Poblacion, numCores, numParts, insertarDatos, transformarEnJSON, nombreColeccionInformacionTaxonomica, nombreColeccionRegistroDeOcurrencia, nombreColeccionPoblacion)