...
###
### Meter objeto InformacionTaxonomica en Poblacion
###
indices <- match(Poblacion$species_LP, InformacionTaxonomica$species_LP)
Poblacion$InformacionTaxonomica <- InformacionTaxonomica[indices, ]


rm(InformacionTaxonomica, RegistroDeOcurrencia, listaRegistrosPoblaciones, indices)
...
rowsPerPart <- ceiling(nrow(Poblacion) / numPartes)
splitData <- split(Poblacion, rep(1:numPartes, each = rowsPerPart, length.out = nrow(Poblacion)))

plan(multisession, workers = numCores)
resultados <- future_map(splitData, transformarEnJSON)


for (json in resultados2) {
  POST("http://localhost:5000/insert", body = json, encode = "json") 
}

rm(RegistroDeOcurrencia, resultados, rowsPerPart, splitData, numCores, numPartes)