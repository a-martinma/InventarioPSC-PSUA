...
rm(dfPoblacion, InventarioNacional, Gestion, MedidasDeConservación, ConservacionExSitu, Links, indices, convertirEnLista)
...
###
## Meter objeto Poblacion en RegistroDeOcurrencia
###
indices <- match(RegistroDeOcurrencia$POPID, Poblacion$POPID)
RegistroDeOcurrencia$Poblacion <- Poblacion[indices, ]

###
### Meter objeto InformacionTaxonomica en RegistroDeOcurrencia
###
indices <- match(RegistroDeOcurrencia$species_LP, InformacionTaxonomica$species_LP)
RegistroDeOcurrencia$InformacionTaxonomica <- InformacionTaxonomica[indices, ] 


rm(indices, InformacionTaxonomica, Poblacion) 
...
rowsPerPart <- ceiling(nrow(RegistroDeOcurrencia) / numPartes)
splitData <- split(RegistroDeOcurrencia, rep(1:numPartes, each = rowsPerPart, length.out = nrow(RegistroDeOcurrencia)))
...
rm(RegistroDeOcurrencia, resultados, json_completo, rowsPerPart, splitData, numCores, numPartes, transformarEnJSON)